-----------------------------------------
-- Tablespace for schema used in aliaisng
-----------------------------------------
...
begin
		...
		la_tb_spaces(1).tbs_name := 'me_data';
		la_tb_spaces(1).tbs_size := '1g';
		la_tb_spaces(1).data_file := '+ORA_DATA';
		
		
		la_tb_spaces(2).tbs_name := 'me_index';
		la_tb_spaces(2).tbs_size := '512M';
		la_tb_spaces(2).data_file := '+ORA_DATA';
		
		for k in 1..la_tb_spaces.count
		loop
			
			--Creating tablespace me_common_index
			dbms_output.put_line('Creating tablespace ' || la_tb_spaces(k).tbs_name);
			
			begin
				lv_tbs_size := 	la_tb_spaces(k).tbs_size;
				if (lv_big_tblspace = upper('N') and upper(substr(lv_tbs_size,-1,1)) = 'G' and to_number(substr(lv_tbs_size,1,length(lv_tbs_size)-1)) > lv_max_tbs_size )
				...
				then
					...
					lv_sql := lv_sql || ' DATAFILE ''' || la_tb_spaces(k).data_file || ''' SIZE ' || la_tb_spaces(k).tbs_size;
					...
					loop				
						...
						if i = 1 then
							lv_sql := 'CREATE  TABLESPACE ' || la_tb_spaces(k).tbs_name;
							lv_sql := lv_sql || ' DATAFILE ''' || la_tb_spaces(k).data_file || ''' SIZE ' || lv_cal_db_file_sz || 'g ';
							lv_sql := lv_sql || ' EXTENT MANAGEMENT LOCAL UNIFORM SIZE 1M';
							lv_sql := lv_sql || ' SEGMENT SPACE MANAGEMENT AUTO';				
						else
							lv_sql := 'ALTER  TABLESPACE ' || la_tb_spaces(k).tbs_name;
							lv_sql := lv_sql || ' ADD DATAFILE SIZE ' || lv_cal_db_file_sz || 'g ';
						end if;
						execute immediate lv_sql;
